...
for xx in 1:length(s)
    for yy in 1:length(phi)
        ...
        # definition of the W_R as <Aligned> 
        for iter in 1:iter_num
            d = Normal(0, s[xx])
            for i in 1:n
                for j in 1:m
                    W_R[i,j] = -(W[i,j] + abs(rand(d,1)[1]))
                end
            end

            for i in 1:n
                for j in 1:m
                    W_R[i,j] = W_R[i,j]+maximum(-W_R)
                end
            end
        ...
        # ------------------------------------------------ write into file --------------------------------------------------
        open("results_file_best_U_6.txt", "w") do io
            write(io, "lambda = 1, U = 6, W_R is the best case -> W_R[i,j] = -(W + abs(rand(d,1)[1]))")
            write(io, "\n")
